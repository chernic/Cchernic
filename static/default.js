...
jQuery(document).ready(function(w){
    ...
    w('body').append('<div id="jquery-things"><div id="gotop">^ TOP</div></div>');
...
function func_scoll(){
    ...
    if(tmp > jQuery('.navigation').offset().top){
       jQuery('.navigation .current-page').next('a').trigger('click');
    }
}
...
$(document).ready(function(){
    ...
    $('#main').bind('pjax.end', function(){
        ...
        $('#loading').fadeOut();
        $(window).bind('scroll',function(){func_scoll();});
    })
});
...
$.extend($.pjax.showFx,{
    custom: function(data, callback, isCached){
        ...
        var $this = this;
        $this.append(output);
        var curpage = $('.current-page').next('a');
        $('.current-page').removeClass('current-page');
        curpage.addClass('current-page');
        callback && callback.call($this, data, isCached);
    }
});
